(() => {
  ...
  function bindStablePrint() {
    ...
    if (document.documentElement.dataset.mobilePrintDelegatedBound === '1') return;
    document.documentElement.dataset.mobilePrintDelegatedBound = '1';

    document.addEventListener('click', (event) => {
      const button = event.target instanceof Element ? event.target.closest('button') : null;
      if (!button) return;
      if (!(button.id === 'print-report' || text(button) === 'Print Report')) return;
      ...
      // Force a layout flush so print captures the report-mode DOM in this same user gesture.
      if (reportView) void reportView.offsetHeight;
      ...
      const markBeforePrint = () => {
        beforePrintSeen = true;
      };
      window.addEventListener('beforeprint', markBeforePrint, { once: true });
      ...
      window.setTimeout(() => {
        if (!beforePrintSeen) {
          try {
            ...
            // Ignore fallback errors and rely on timeout cleanup.
          }
        }
      }, 200);
      ...
      try {
        ...
        // Some engines throw intermittently; the fallback above retries once.
      }
    }, true);
  }
