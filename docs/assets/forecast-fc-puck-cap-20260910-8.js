(() => {
  ...
  function ensureForecastOptionRows() {
    ...
    const cyaMin = parseRangeMin(document.getElementById('range-cya')?.textContent, 30);
    const cyaMax = parseRangeMax(document.getElementById('range-cya')?.textContent, 80);
    const chMin = parseRangeMin(document.getElementById('range-ch')?.textContent, 200);
  ...
  function patchForecastCyaLine() {
    ...
    if (Number.isFinite(cyaMax) && cya > cyaMax) {
      setChecklistLineText(cyaLineItem, `CYA: High at ${Math.round(cya)} ppm (target: ${Math.round(cyaMin)}-${Math.round(cyaMax)} ppm). Choose one option below.`);
      return;
    }
