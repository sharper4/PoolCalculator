(() => {
  ...
  function updateGoalNote() {
    ...
    const noteText = `Shock and SLAM use the same FC level here: ${shock} ppm at the current CYA. Reach it with liquid chlorine, then test and re-dose often enough to hold that FC until the water is clear, combined chlorine is 0.5 ppm or less, and overnight FC loss is 1 ppm or less.`;
    ...
    const mustardLine = document.createElement('div');
    mustardLine.dataset.goalNote = '1';
    mustardLine.textContent = `Mustard algae cleanup is ${mustard} ppm after SLAM is complete.`;
    goalResult.appendChild(mustardLine);
  }
